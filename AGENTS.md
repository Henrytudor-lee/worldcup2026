...
**关键设计原则**：用户对算法的 16 个系数有**完全控制权**——UI 滑块改完要能立即在客户端/服务端重算并刷新预测。
...
└── 5_算法/                                  ← 复现算法
    ...
    ├── weights_v21.json                         16 系数 JSON（嵌入 SPA）
    └── (其他 .py 辅助脚本)
```
