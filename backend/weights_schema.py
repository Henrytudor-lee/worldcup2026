...
DEFAULT = {
    ...
    "lambda_cap": 3.5,  # λ 上限 (v2.2.4-8 2.8→3.5, 2.8 太低导致 60% KO 场是 2-2 → 12/16 走点球逻辑, 离谱)
    "draw_boost": 1.35,  # 平局概率加成 (v2.2.4, 1.0=不加权, 1.35=+35%; 修'身价压制过度')
    # ============ v3.0 KO 阶段调整 (7/2 加) ============
    "knockout_lambda_reducer": 0.85,  # KO 阶段 λ 整体降 15% (32 强实测 KO λ 比小组高估, 降下来)
    "extra_time_prob": 0.20,  # KO 加时/点球概率加成 (基础 +20% 到 P(平局), 实际 10 场有 3 场进点球 = 30%)
    "red_card_penalty": 0.70,  # 红牌后 λ 乘数 (实际少打 1 人 ≈ -30% 进攻效率)
    "extra_time_min_draw_prob": 0.25,  # KO P(平) 至少 25% (强制保证加时场景可见)
}
...
RANGES = {
    ...
    "knockout_lambda_reducer": (0.6, 1.0),  # KO λ 减系数 (1.0=不降, 0.6=降 40%)
    "extra_time_prob": (0.0, 0.5),  # KO 加时概率加成 (0=不加, 0.5=最大 +50%)
    "red_card_penalty": (0.5, 1.0),  # 红牌 λ 系数 (1.0=不影响, 0.5=减半)
    "extra_time_min_draw_prob": (0.0, 0.5),  # KO P(平) 下限
}
